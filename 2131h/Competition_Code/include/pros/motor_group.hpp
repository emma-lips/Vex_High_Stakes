...
class MotorGroup : public virtual AbstractMotor
{
 ...
 public:
   ...
   *
   * \param port
   *        A initializer list of V5 port numbers from 1 to 21, or from -21 to
   * -1 for reversed motors. A reversed motor will reverse the input or output
   * movement functions and movement related telemetry in order to produce
   * consistant behavior with non-reversed motors
   *
   * \param gearset =  pros::v5::MotorGears::invalid
   * 		  Optional parameter for the gearset for the motor.
   * 		  Does not explicitly set the motor gearset if it is invalid or
   * not specified
   *
   * \param encoder_units = pros::v5::MotorUnits::invalid
   * 		  Optional parameter for the encoder units of the motor
   * 		  Does not explicitly set the motor units if it is invalid or
   * not specified
   *
   *  \b Example
   * \code
   * void opcontrol() {
   * 	MotorGroup first_mg({1, -2}); //Creates a motor on port 1  and a
   * reversed motor on port 2 MotorGroup rotations_mg({4, 5},
   * pros::v5::MotorGears::blue, pros::v5::MotorUnits::rotations);
   *  //Creates a motor group on ports 4 and 5 with blue motors using rotaions
   ...
   * void opcontrol() {
   * 	MotorGroup first_mg({1, -2}); //Creates a motor on port 1  and a
   * reversed motor on port 2 with with both motors using the green gearset and
   * degrees as the encoder units MotorGroup rotations_mg({4, 5},
   * pros::v5::MotorGears::blue, pros::v5::MotorUnits::rotations);
   *  //Creates a motor group on ports 4 and 5 with blue motors using rotaions
   * as the encoder units
   * }
   ...
   * 	MotorGroup first_mg({1, -2}); //Creates a motor on port 1  and a
   * reversed motor on port 2 with with both motors using the green gearset and
   * degrees as the encoder units AbstractMotor abs_mtr_group = first_mg;
   ...
   *     // Continue running this loop as long as the mg is not within +-5 units
   * of its goal pros::delay(2);
   *   }
   ...
   *   while (!((mg.get_position() < 105) && (mg.get_position() > 95))) {
   *     // Continue running this loop as long as the motor is not within +-5
   * units of its goal pros::delay(2);
   *   }
  ...
  /**
   * Sets the output voltage for the motor group from -12000 to 12000 in
   * millivolts.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   ...
   * move_relative). This will have no effect if the motor group is not
  ...
  /**
   * Gets the target position set for a motor in the motor group, with a
   * parameter for the motor index.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENODEV - The port cannot be configured as a motor
   ...
   * 				The zero indexed index of the motor in the motor
   * group
   *
   ...
   * EDOM - The Motor group is empty
   *
   * \return The a vector of the target positions in its encoder units or
  ...
  /**
   * Gets the velocity commanded to the motor by the user at the index
   * specified.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   ...
   * EDOM - THe motor group is empty
   *
   * \return A vector of the commanded motor velocity from +-100, +-200, or
   * +-600, or PROS_ERR if the operation failed, setting errno.
   *
   * \b Example
   * \code
   * void opcontrol() {
   ...
   *   pros::MotorGroup mg({1,3});
   *   while (true) {
   *     mg = controller_get_analog(E_CONTROLLER_MASTER,
   ...
   * EDOM - THe motor group is empty
   *
   * \return A vector of the each motor's actual velocity in RPM or PROS_ERR_F
   ...
   *
   *
   * \return A vector with each motor's current in mA or PROS_ERR if the
   * operation failed, setting errno.
   *
   * \b Example
   * \code
  ...
  /**
   * Gets a vector of the directions of movement for each motor in the motor
   * group.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   ...
   * EDOM - THe motor group is empty
   *
   * \return A vector containing each motor's efficiency in percent or
   * PROS_ERR_F if the operation failed, setting errno.
   *
   * \b Example
   * \code
  ...
  /**
   * Gets a vector of the flags set by each motor in the motor groups's
   * operation.
   *
   * Compare this bitfield to the bitmasks in pros::motor_flag_e_t.
   *
  ...
  /**
   * Gets the absolute position of a motor in the motor group in its encoder
   * units.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   ...
   * EDOM - The motor group is empty
   *
   * \return A vector of the motor's absolute position in its encoder units or
   * PROS_ERR_F if the operation failed, setting errno.
   *
   * \b Example
   * \code
   ...
   * EDOM - The motor group is empty
   *
   * \return A vector of each motor's power draw in Watts or PROS_ERR_F if the
   * operation failed, setting errno.
   *
   * \b Example
   * \code
  ...
  /**
   * Gets the raw encoder count of a motor in the motor group at a given
   * timestamp.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   ...
   *            count was read will not be supplied
   *
   * \return A vector of each raw encoder count at the given timestamp or
   * PROS_ERR if the operation failed.
   *
   * \b Example
   * \code
   * void opcontrol() {
   ...
   * \endcode
   */
  std::vector<std::int32_t> get_raw_position_all(
   ...
   * EDOM - The motor group is empty
   *
   * \return A vector of each motor's temperature in degrees Celsius or
   * PROS_ERR_F if the operation failed, setting errno.
   *
   * \b Example
   * \code
  ...
  /**
   * Gets the torque generated by a motor in the motor groupin Newton Meters
   * (Nm).
   *
   * This function uses the following values of errno when an error state is
   * reached:
   ...
   * EDOM - The motor group is empty
   *
   * \return A vector containing each motor's torque in Nm or PROS_ERR_F if the
   * operation failed, setting errno.
   *
   * \b Example
   * \code
   ...
   * EDOM - The motor group is empty
   *
   * \return A vector of each motor's voltage in mV or PROS_ERR_F if the
   * operation failed, setting errno.
   *
   * \b Example
   * \code
   ...
   *   while (true) {
   *     mg = master.get_analog(E_CONTROLLER_ANALOG_LEFT_Y);
   *     std::cout << "Is the motor over its current limit?: " <<
   * mg.is_over_current(); pros::delay(2);
   *   }
   * }
   * \endcode
   */
   ...
   * EDOM - The motor group is empty
   *
   * \return A vector containing the following for each motor: 1 if the motor's
   * current limit is being exceeded and 0 if the current limit is not exceeded,
   ...
   *     mg = master.get_analog(E_CONTROLLER_ANALOG_LEFT_Y);
   *     std::cout << "Is the motor over its current limit?: " <<
   * motor.is_over_current_all()[0]; pros::delay(2);
   *   }
   * }
   * \endcode
   */
   ...
   *   while (true) {
   *     mg = master.get_analog(E_CONTROLLER_ANALOG_LEFT_Y);
   *     std::cout << "Is the motor over its temperature limit?: " <<
   * motor.is_over_temp(); pros::delay(2);
   *   }
   * }
   * \endcode
   */
  ...
  /**
   * Gets a vector with the temperature limit flag for each motor in the motor
   * group.
   *
   * This function uses the following values of errno when an error state is
   * reached:
  ...
  /**
   * Gets a vector with the brake mode that was set for each motor in the motor
   * group.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   ...
   * EDOM - The motor group is empty
   *
   * \return A vector with one of MotorBrake for each motor in the motor group,
   * according to what was set for the motor, or E_MOTOR_BRAKE_INVALID if the
   * operation failed, setting errno.
   *
   * \b Example
   * \code
   ...
   * EDOM - The motor group is empty
   *
   * \return A vector of each motor's current limit in mA or PROS_ERR if the
   * operation failed, setting errno.
   *
   * \b Example
   * \code
   ...
   * \code
   * void initialize() {
   *   pros::MotorGroup mg ({1,3}, E_MOTOR_GEARSET_06, false,
   * E_MOTOR_ENCODER_COUNTS); std::cout << "Motor Encoder Units: " <<
   * mg.get_encoder_units();
   * }
   * \endcode
   */
  ...
  /**
   * Gets a vector of the encoder units that were set for each motor in the
   * motor group.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   ...
   * EDOM - The motor group is empty
   *
   * \return A vector with the following for each motor, One of MotorUnits
   * according to what is set for the motor or E_MOTOR_ENCODER_INVALID if the
   * operation failed.
   *
   * \b Example
   * \code
   ...
   * void initialize() {
   *   pros::MotorGroup mg ({1,3}, E_MOTOR_GEARSET_06, false,
   *E_MOTOR_ENCODER_COUNTS); std::cout << "Motor Gearing: " << mg.get_gearing();
   * }
   * \endcode
   */
   ...
   *
   *
   * \return A vector with one of MotorGears according to what is set for the
   * motor, or pros::MotorGears::invalid if the operation failed for each motor.
   *
   * \b Example
   * \code
   * void initialize() {
   *   pros::MotorGroup mg ({1,3}, E_MOTOR_GEARSET_06, false,
   * E_MOTOR_ENCODER_COUNTS); std::cout << "Motor Gearing: " <<
   * mg.get_gearing_all()[0];
   * }
   * \endcode
   */
  ...
  /**
   * Gets the operation direction of a motor in the motor group as set by the
   *user.
   *
   * This function uses the following values of errno when an error state is
   * reached:
  ...
  /**
   * Gets a vector of the operation direction of each motor in the motor group
   * as set by the user.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * EDOM - The motor group is empty
   *
   * \return A vector conatining the following for each motor: 1 if the motor
   * has been reversed and 0 if the motor was not reversed, or PROS_ERR if the
   * operation failed, setting errno.
   *
   * \b Example
   * \code
  ...
  std::int32_t set_brake_mode(const pros::motor_brake_mode_e_t mode,
                              const std::uint8_t index = 0) const;
  /**
   * Sets one of MotorBrake all the motors in the motor group. Works with the C
   ...
   *   // The motor will reduce its output at 1000 mA instead of the default
   * 2500 mA pros::delay(2);
   *   }
   * }
   * \endcode
   */
   ...
   * \endcode
   */
  std::int32_t set_encoder_units_all(
      const pros::motor_encoder_units_e_t units) const;

  /**
   * Sets one of the gear cartridge (red, green, blue) for one motor in the
   * motor group. Usable with the C++ enum class and the C enum.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENODEV - The port cannot be configured as a motor
   * EDOM - The motor group is empty
   * E2BIG - The size of the vector mismatches the number of motors in the motor
   * group
   *
   * \note If there are more motors than gearsets passed in,
   * 	only the first n motors will have their gearsets changed where n is the
   * number of gearsets passed in. If there are more gearsets passed in than
   * motors, then the only the first m gearsets will be used, where m is the
   * number of motors. In either case, errno will be set to E2BIG, but the
  ...
  /**
   * Sets the gear cartridge (red, green, blue) for each motor in the motor
   * group by taking in a vector of the cartridges. Usable with the C++ enum
   * class and the C enum.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENODEV - The port cannot be configured as a motor
  ...
  /**
   * Sets one of the gear cartridge (red, green, blue) for every motor in the
   * motor group. Usable with the C++ enum class and the C enum.
   *
   * This function uses the following values of errno when an error state is
   * reached:
  ...
  /**
   * Sets the "absolute" zero position of a motor in the motor group to its
   * current position.
   *
   * This function uses the following values of errno when an error state is
   * reached:
  ...
  /**
   * Sets the "absolute" zero position of every motor in the motor group to its
   * current position.
   *
   * This function uses the following values of errno when an error state is
   * reached:
  ...
  /**
   * Appends all the motors in the other motor group reference to this motor
   * group
   *
  ...
  /**
   * Removes the all motors on the port (regardless of reversal) from the motor
   * group
   *
