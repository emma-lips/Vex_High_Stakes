...
void worldssixdonutsrightredalliance() {
  ...
  chassis.drive_brake_set(MOTOR_BRAKE_COAST);
  chassis.pid_drive_set(51, FULL_SPEED, true); //Drive to ladder
  chassis.pid_wait();
...
void worldssixdonutsrightredelims() {
  ...
  chassis.pid_drive_set(-26, 70);//CHANGE IF SWING NO HIT
  ...
  doinkerright.extend();
  setIntake(127);
  chassis.pid_drive_set(9, 100); //Get 6 donut
  chassis.pid_wait_quick_chain();
...
void worldssixdonutsleftblueelims() {
  ...
  chassis.pid_turn_relative_set(-90, 127);
  chassis.pid_wait_quick_chain();
    // chassis.pid_turn_set(-90, 80); //clear corners
  // chassis.pid_wait_quick_chain();
